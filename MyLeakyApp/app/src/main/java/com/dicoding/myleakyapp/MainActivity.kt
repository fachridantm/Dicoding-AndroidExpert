...
class MainActivity : AppCompatActivity() {
    ...
    private fun registerBroadcastReceiver() {
        broadcastReceiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context, intent: Intent) {
                when (intent.action) {
                    Intent.ACTION_POWER_CONNECTED -> tvPowerStatus.text =
                        getString(R.string.power_connected)
                    Intent.ACTION_POWER_DISCONNECTED -> tvPowerStatus.text =
    ...
    // Don't forget to unregister the broadcast receiver onStop lifecycle method
    override fun onStop() {
        super.onStop()
